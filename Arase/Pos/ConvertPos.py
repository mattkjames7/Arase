import numpy as np
from .ReadPosCDF import ReadPosCDF
...
def _MagGeo(xm,ym,zm,Date,ut):
	...
	glon,glat = gp.MAGtoGEOUT(mlon,mlat,Date,ut)
...
def ConvertPos():
	...
	for i in range(0,nd):
		print('\rReading Date {0} of {1}'.format(i+1,nd),end='')
		#read cdf
		tmp = ReadPosCDF(dates[i],2)
		...
		if not tmp is None:
			...
			for j in range(0,1440):
				out.Xgm[p+j],out.Ygm[p+j],out.Zgm[p+j] = gp.GSEtoMAGUT(out.Xgse[p+j],out.Ygse[p+j],out.Zgse[p+j],dates[i],ut[j])
